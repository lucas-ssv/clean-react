...
const makeSut = ({ loadSurveyResultSpy = new LoadSurveyResultSpy(), saveSurveyResultSpy = new SaveSurveyResultSpy(), initialState }: SutParams = {}): SutTypes => {
...
describe('SurveyResultComponent', () => {
  ...
  test('Should prevent multiple answer click', async () => {
    ...
    await waitFor(() => screen.getByTestId('survey-result'))
    const answersWrap = screen.getAllByTestId('answer-wrap')
